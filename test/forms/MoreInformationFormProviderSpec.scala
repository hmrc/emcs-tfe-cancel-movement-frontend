...
class MoreInformationFormProviderSpec extends FormSpec with GuiceOneAppPerSuite {

  val requiredKey = "moreInformation.error.required"
  val lengthKey = "moreInformation.error.length"
  val xssKey = "moreInformation.error.xss"
  val characterKey = "moreInformation.error.character"
  val maxLength = 350
  val fieldName = "value"
  ...
  ".value" - {
    ...
    "when value is mandatory" - {
      ...
      val form = new MoreInformationFormProvider().apply(isMandatory = true)
    ...
    "when value is NOT mandatory" - {
      ...
      val form = new MoreInformationFormProvider().apply(isMandatory = false)
