...
class ChooseGiveMoreInformationController @Inject()(
                                                     ...
                                                     formProvider: ChooseGiveMoreInformationFormProvider,
                                                     val controllerComponents: MessagesControllerComponents,
                                                     view: ChooseGiveMoreInformationView
                                                   ) extends BaseNavigationController with AuthActionHelper {

  def onPageLoad(ern: String, arc: String, mode: Mode): Action[AnyContent] =
    authorisedDataRequestWithCachedMovementAsync(ern, arc) { implicit request =>
      renderView(Ok, fillForm(ChooseGiveMoreInformationPage, formProvider()), mode)
    }
  ...
  private def renderView(status: Status, form: Form[_], mode: Mode)(implicit request: DataRequest[_]): Future[Result] =
    Future.successful(status(view(form, routes.ChooseGiveMoreInformationController.onSubmit(request.ern, request.arc, mode))))
}
