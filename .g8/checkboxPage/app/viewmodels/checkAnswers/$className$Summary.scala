...
object $className$Summary  {
  ...
  def row(userAnswers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    userAnswers.get($className$Page).map {
      answers =>
        ...
        SummaryListRowViewModel(
          ...
          value   = value,
          actions = Seq(
            ActionItemViewModel("site.change", routes.$className$Controller.onPageLoad(userAnswers.ern, userAnswers.arc, CheckMode).url, $className$Page)
